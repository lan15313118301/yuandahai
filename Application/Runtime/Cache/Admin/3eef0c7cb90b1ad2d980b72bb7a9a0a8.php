...
<script type="text/javascript">
	function del($cm_id){
		if(confirm("确定要删除此项分类吗？")){
			window.location="/index.php/Admin/Modcompany/del/cmid/"+$cm_id+".html";
		}
	}
	function getNum(){
	...
	function alldel(){
		...
		if(confirm("您确定要删除选中的栏目信息吗！！")){
			window.frm.action="/index.php/Admin/Modcompany/delall";
			document.frm.submit();
		}
	}
	function showpai(){
	...
	function allpai(){
		window.frm.action="/index.php/Admin/Modcompany/allpai";
		document.frm.submit();
	}
</script>
</head>
	...
	function nologin(){
	    ...
	    <div id="page-wrapper">
		        <div class="graphs">
		        	<div>
						<div class="clearfix">
							<h3 class="content-title">所有产品种类</h3>
						</div>
						<div class="description" ><?php echo ($msg["role"]); ?></div>
						<div class="col-sm-12">
                                <a href="/index.php/Admin/Addcompany/index.html"  class="btn btn-secondary btn-lo pull-right">添加产品种类</a>
                            </div>
							...
							<div class="panel panel-warning" data-widget="{&quot;draggable&quot;: &quot;false&quot;}" data-widget-static="">
									...
									<form name="frm" id='frm' action='' method="post">
										<table class="table table-striped">
											...
											<tbody>
											<?php if(is_array($company)): foreach($company as $key=>$v): ?><tr>
													<td><input type="checkbox" name="news[]" onclick='newsdel()' value="<?php echo ($v["cm_id"]); ?>"/></td>
													<td><?php echo ($v["cm_id"]); ?></td>
													<td><input type="text" size="3"  onchange='showpai()' name="paixu[<?php echo ($v["cm_id"]); ?>]" value="<?php echo ($v["cm_pai"]); ?>"/></td>
													<td><?php echo (cutstr($v["cm_name"],27)); ?></td>
													<td><?php echo (cutstr($v["encm_name"],27)); ?></td>
													<td><?php echo (date('Y-m-d H:m',$v["cm_time"])); ?></td>
													<?php if($v["cm_upttime"] == null): ?><td>无</td>
													<?php else: ?>
														<td><?php echo (date('Y-m-d H:m',$v["cm_upttime"])); ?></td><?php endif; ?>
													<td>
														<a href="/index.php/Admin/Modcompany/upt/cmid/<?php echo ($v["cm_id"]); ?>.html" class="btn btn-default btn-sm btn-lo">修改</a>
                                          				  <a href="#" onclick="del(<?php echo ($v["cm_id"]); ?>)" class="btn btn-default btn-sm btn-lo">删除</a>
													</td>
