...
<script type="text/javascript">
	function del($c_id){
		if(confirm("确定要删除此产品种类和其产品吗？")){
			window.location="/index.php/Admin/Modcate/del/cid/"+$c_id+".html";
		}
	}
	function getNum(){
	...
	function alldel(){
		...
		if(confirm("您确定要删除选中项目及相应产品？请慎重！！")){
			window.frm.action="/index.php/Admin/Modcate/delall";
			document.frm.submit();
		}
	}
	function showpai(){
	...
	function allpai(){
		window.frm.action="/index.php/Admin/Modcate/allpai";
		document.frm.submit();
	}
</script>
</head>
	    ...
	    <div id="page-wrapper">
		        <div class="graphs">
		        	<div>
						...
						<div class="col-sm-12">
                                <a href="/index.php/Admin/Addcate/index.html"  class="btn btn-secondary btn-lo pull-right">添加产品种类</a>
                            </div>
									...
									<form name="frm" id='frm' action='' method="post">
										<table class="table table-striped">
											...
											<?php if(is_array($cate)): foreach($cate as $key=>$v): ?><tr>
													<td><input type="checkbox" name="news[]" onclick='newsdel()' value="<?php echo ($v["c_id"]); ?>"/></td>
													<td><?php echo ($v["c_id"]); ?></td>
													<td><input type="text" size="3"  onchange='showpai()' name="paixu[<?php echo ($v["c_id"]); ?>]" value="<?php echo ($v["c_pai"]); ?>"/></td>
													<td><?php echo (cutstr($v["c_name"],27)); ?></td>
													<td><?php echo (cutstr($v["enc_name"],27)); ?></td>
													<td><?php echo (date('Y-m-d H:m',$v["c_time"])); ?></td>
													<?php if($v["c_upttime"] == null): ?><td>无</td>
													<?php else: ?>
														<td><?php echo (date('Y-m-d H:m',$v["c_upttime"])); ?></td><?php endif; ?>
													<td>
														<a href="/index.php/Admin/Modcate/upt/cateId/<?php echo ($v["c_id"]); ?>.html" class="btn btn-default btn-sm btn-lo">修改</a>
                                          				  <a href="#" onclick="del(<?php echo ($v["c_id"]); ?>)" class="btn btn-default btn-sm btn-lo">删除</a>
													</td>
