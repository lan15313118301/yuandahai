...
<script type="text/javascript">
	function del($n_id){
		if(confirm("确定要删除这条新闻吗？")){
			window.location="/index.php/Admin/Modnews/del/nid/"+$n_id;
		}
	}
	function getNum(){
	...
	function alldel(){
		...
		window.frm.action="/index.php/Admin/Modnews/delall";
		document.frm.submit();
	}
	function showpai(){
	...
	function allpai(){
		window.frm.action="/index.php/Admin/Modnews/allpai";
		document.frm.submit();
	}
</script>
</head>
	...
	function nologin(){
	    ...
	    <div id="page-wrapper">
		        <div class="graphs">
		        	<div>
						<div class="clearfix">
							<h3 class="content-title">所有新闻</h3>
						</div>
						<div class="description" ><?php echo ($msg["role"]); ?></div>
						<div class="col-sm-12">
                                <a href="/index.php/Admin/Addnews/index.html"  class="btn btn-secondary btn-lo pull-right">添加新闻</a>
                            </div>
									...
									<form name="frm" id='frm' action='' method="post">
										<table class="table table-striped">
											...
											<?php if(is_array($news)): foreach($news as $key=>$v): ?><tr>
													<td><input type="checkbox" name="news[]" onclick='newsdel()' value="<?php echo ($v["n_id"]); ?>"/></td>
													<td><?php echo ($v["n_id"]); ?></td>
													<td><input type="text" size="2"  onchange='showpai()' name="paixu[<?php echo ($v["n_id"]); ?>]" value="<?php echo ($v["n_pai"]); ?>"/></td>
													<td><?php echo (cutstr($v["n_title"],10)); ?></td>
													<td>
													<?php if($v["n_picture"] != '0'): ?><img src='/public/images/<?php echo ($v["n_picture"]); ?>' alt='' title='' width="40" height="20"/><?php endif; ?>	
													</td>
													<td><?php echo (cutstr($v["enn_title"],10)); ?></td>
													<td>
													<?php if($v["enn_picture"] != '0'): ?><img src='/public/images/<?php echo ($v["enn_picture"]); ?>' alt='' title='' width="40" height="20"/><?php endif; ?>	
													</td>
													<td><?php echo ($v["n_cate"]); ?></td>
													<td><?php echo (date('Y-m-d H:m',$v["n_time"])); ?></td>
													
													<td><?php echo ($v["n_click"]); ?></td>
													<td>
														<a href="/index.php/Admin/Modnews/upt/newsId/<?php echo ($v["n_id"]); ?>" class="btn btn-default btn-sm btn-lo">修改</a>
                                          				  <a href="#" onclick="del(<?php echo ($v["n_id"]); ?>)" class="btn btn-default btn-sm btn-lo">删除</a>
													</td>
