...
<script type="text/javascript">
	function del($b_id){
		if(confirm("确定要删除这条大图吗？")){
			window.location="/index.php/Admin/Modbanner/del/bid/"+$b_id;
		}
	}
	function getNum(){
	...
	function alldel(){
		...
		window.frm.action="/index.php/Admin/Modbanner/delall";
		document.frm.submit();
	}
	function showpai(){
	...
	function allpai(){
		window.frm.action="/index.php/Admin/Modbanner/allpai";
		document.frm.submit();
	}
</script>
</head>
	...
	function nologin(){
	    ...
	    <div id="page-wrapper">
		        <div class="graphs">
		        	<div>
						<div class="clearfix">
							<h3 class="content-title">所有导航大图</h3>
						</div>
						<div class="description" ><?php echo ($msg["role"]); ?></div>
						<div class="col-sm-12">
                                <a href="/index.php/Admin/Addbanner/index.html"  class="btn btn-secondary btn-lo pull-right">添加导航大图</a>
                            </div>
									...
									<form name="frm" id='frm' action='' method="post">
										<table class="table table-striped">
											...
											<?php if(is_array($banner)): foreach($banner as $key=>$v): ?><tr>
													<td><input type="checkbox" name="news[]" onclick='newsdel()' value="<?php echo ($v["b_id"]); ?>"/></td>
													<td><?php echo ($v["b_id"]); ?></td>
													<td><input type="text" size="2"  onchange='showpai()' name="paixu[<?php echo ($v["b_id"]); ?>]" value="<?php echo ($v["b_pai"]); ?>"/></td>
													<td>
													<?php if($v["b_picture"] != '0'): ?><img src='/Public/images/<?php echo ($v["b_picture"]); ?>' alt='' title='' width="300" height="150"/><?php endif; ?>	
													</td>
													<td>
													<?php if($v["enb_picture"] != '0'): ?><img src='/Public/images/<?php echo ($v["enb_picture"]); ?>' alt='' title='' width="300" height="150"/><?php endif; ?>	
													</td>
													<td><?php echo (date('Y-m-d H:m',$v["b_time"])); ?></td>
													<td>
														<a href="/index.php/Admin/Modbanner/upt/bid/<?php echo ($v["b_id"]); ?>.html" class="btn btn-default btn-sm btn-lo">修改</a>
                                          				  <a href="#" onclick="del(<?php echo ($v["b_id"]); ?>)" class="btn btn-default btn-sm btn-lo">删除</a>
													</td>
