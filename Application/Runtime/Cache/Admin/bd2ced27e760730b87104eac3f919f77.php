...
<script type="text/javascript">
	function del($j_id){
		if(confirm("确定要删除此项岗位需求吗？")){
			window.location="/index.php/Admin/Modjob/del/jid/"+$j_id+".html";
		}
	}
	function getNum(){
	...
	function alldel(){
		...
		if(confirm("您确定要删除选中的招聘信息吗！！")){
			window.frm.action="/index.php/Admin/Modjob/delall";
			document.frm.submit();
		}
	}
	function showpai(){
	...
	function allpai(){
		window.frm.action="/index.php/Admin/Modjob/allpai";
		document.frm.submit();
	}
</script>
</head>
	    ...
	    <div id="page-wrapper">
		        <div class="graphs">
		        	<div>
						...
						<div class="col-sm-12">
                                <a href="/index.php/Admin/Addjob/index.html"  class="btn btn-secondary btn-lo pull-right">添加招聘信息</a>
                            </div>
							...
							<div class="panel panel-warning" data-widget="{&quot;draggable&quot;: &quot;false&quot;}" data-widget-static="">
									...
									<form name="frm" id='frm' action='' method="post">
										<table class="table table-striped">
											...
											<tbody>
											<?php if(is_array($job)): foreach($job as $key=>$v): ?><tr>
													<td><input type="checkbox" name="news[]" onclick='newsdel()' value="<?php echo ($v["j_id"]); ?>"/></td>
													<td><?php echo ($v["j_id"]); ?></td>
													<td><input type="text" size="3"  onchange='showpai()' name="paixu[<?php echo ($v["j_id"]); ?>]" value="<?php echo ($v["j_pai"]); ?>"/></td>
													<td><?php echo (cutstr($v["j_name"],20)); ?></td>
													<td><?php echo (cutstr($v["enj_name"],30)); ?></td>
													<td><?php echo (date('Y-m-d H:m',$v["j_time"])); ?></td>
													<?php if($v["j_upttime"] == null): ?><td>无</td>
													<?php else: ?>
														<td><?php echo (date('Y-m-d H:m',$v["j_upttime"])); ?></td><?php endif; ?>
													<td>
														<a href="/index.php/Admin/Modjob/upt/jid/<?php echo ($v["j_id"]); ?>.html" class="btn btn-default btn-sm btn-lo">修改</a>
                                          				  <a href="#" onclick="del(<?php echo ($v["j_id"]); ?>)" class="btn btn-default btn-sm btn-lo">删除</a>
													</td>
