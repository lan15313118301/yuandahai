...
<script type="text/javascript">
	function del($mid){
		if(confirm("确定要彻底删除数据吗？")){
			window.location="/index.php/Admin/Showmessage/truedel/mid/"+$mid+".html";
		}
	}
	function getNum(){
	...
	function alldel(){
		...
		if(confirm("确定要删除所有选中的留言吗？")){
			window.frm.action="/index.php/Admin/Showmessage/deltruemess.html";
			document.frm.submit();
		}
	}
	function showpai(){
	...
	function allpai(){
		window.frm.action="/index.php/Admin/Modmessage/allpai";
		document.frm.submit();
	}
</script>
</head>
	...
	function nologin(){
		if(confirm("您确定要退出后台吗？")){
            ...
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="/index.php/Admin/Index/index.html">大庆丹诺石油科技开发有限公司后台系统</a>
            </div>
            <!-- /.navbar-header -->
	    ...
	    <div id="page-wrapper">
		        <div class="graphs">
		        	<div>
						<div class="clearfix">
							<h3 class="content-title">回收站</h3>
						</div>
						<div class="description" ><?php echo ($msg["role"]); ?></div>
						<div class="col-sm-12">
                                <a href="/index.php/Admin/Showmessage/index.html"  class="btn btn-secondary btn-lo pull-right">查看客户留言</a>
                            </div>
									...
									<form name="frm" id='frm' action='' method="post">
										<table class="table table-striped">
											...
											<?php if(is_array($message)): $i = 0; $__LIST__ = $message;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
													<td><input type="checkbox" name="news[]" onclick='newsdel()' value="<?php echo ($v["id"]); ?>"/></td>
													<td><?php echo ($i); ?></td>
													<td><?php echo (cutstr($v["name"],27)); ?></td>
													<td><?php echo ($v["dianhua"]); ?></td>
													<td><?php echo (date('Y-m-d H:m',$v["m_time"])); ?></td>
													<td>
														<a href="/index.php/Admin/Showmessage/huanyuan/mid/<?php echo ($v["id"]); ?>.html" class="btn btn-default btn-sm btn-lo">还原</a>
                                          				  <a href="#" onclick="del(<?php echo ($v["id"]); ?>)" class="btn btn-default btn-sm btn-lo">彻底删除</a>
													</td>
