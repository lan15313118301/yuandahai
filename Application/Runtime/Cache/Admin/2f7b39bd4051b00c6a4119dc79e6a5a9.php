...
<script type="text/javascript">
	function del($q_id){
		if(confirm("栏目禁止删除！")){
			window.location="/index.php/Admin/Modqita/index.html";
		}
	}
	function getNum(){
	...
	function alldel(){
		...
		if(confirm("栏目禁止删除！")){
			window.frm.action="/index.php/Admin/Modqita/index.html";
			document.frm.submit();
		}
	}
	function showpai(){
	...
	function allpai(){
		window.frm.action="/index.php/Admin/Modqita/allpai";
		document.frm.submit();
	}
</script>
</head>
	...
	function nologin(){
	    ...
	    <div id="page-wrapper">
		        <div class="graphs">
		        	<div>
						<div class="clearfix">
							<h3 class="content-title">所有栏目展示</h3>
						</div>
						<div class="description" ><?php echo ($msg["role"]); ?></div>
						<div class="col-sm-12">
                                <a href="/index.php/Admin/Addqita/index.html"  class="btn btn-secondary btn-lo pull-right">添加新的栏目</a>
                            </div>
							...
							<div class="panel panel-warning" data-widget="{&quot;draggable&quot;: &quot;false&quot;}" data-widget-static="">
									...
									<form name="frm" id='frm' action='' method="post">
										<table class="table table-striped">
											...
											<tbody>
											<?php if(is_array($qita)): foreach($qita as $key=>$v): ?><tr>
													<td><input type="checkbox" name="news[]" onclick='newsdel()' value="<?php echo ($v["q_id"]); ?>"/></td>
													<td><?php echo ($v["q_id"]); ?></td>
													<td><?php echo (cutstr($v["q_name"],27)); ?></td>
													<td><?php echo (cutstr($v["enq_name"],27)); ?></td>
													<td><?php echo (date('Y-m-d H:m',$v["q_time"])); ?></td>
													<?php if($v["q_upttime"] == null): ?><td>无</td>
													<?php else: ?>
														<td><?php echo (date('Y-m-d H:m',$v["q_upttime"])); ?></td><?php endif; ?>
													<td>
														<a href="/index.php/Admin/Modqita/upt/qid/<?php echo ($v["q_id"]); ?>.html" class="btn btn-default btn-sm btn-lo">修改</a>
                                          				  <a href="#" onclick="del(<?php echo ($v["q_id"]); ?>)" class="btn btn-default btn-sm btn-lo">删除</a>
													</td>
