...
<script type="text/javascript">
	function del($mid){
		if(confirm("确定要将客户留言放入回收站吗？")){
			window.location="/index.php/Admin/Showmessage/del/mid/"+$mid+".html";
		}
	}
	function getNum(){
	...
	function alldel(){
		...
		if(confirm("确定将选中的留言信息放入回收站吗！")){
			window.frm.action="/index.php/Admin/Showmessage/changeall.html";
			document.frm.submit();
		}
	}
	function showpai(){
	...
	function nologin(){
	    ...
	    <div id="page-wrapper">
		        <div class="graphs">
		        	<div>
						<div class="clearfix">
							<h3 class="content-title">所有留言信息</h3>
						</div>
						<div class="description" ><?php echo ($msg["role"]); ?></div>
						<div class="col-sm-12">
                                <a href="/index.php/Admin/Showmessage/huishou.html"  class="btn btn-secondary btn-lo pull-right">查看回收站</a>
                            </div>
									...
									<form name="frm" id='frm' action='' method="post">
										<table class="table table-striped">
											...
											<?php if(is_array($message)): $i = 0; $__LIST__ = $message;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
													...
													<td>
														<a href="/index.php/Admin/Showmessage/upt/mid/<?php echo ($v["id"]); ?>.html" class="btn btn-default btn-sm btn-lo">查看</a>
                                          				  <a href="#" onclick="del(<?php echo ($v["id"]); ?>)" class="btn btn-default btn-sm btn-lo">删除</a>
													</td>
